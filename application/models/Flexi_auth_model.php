...
class Flexi_auth_model extends Flexi_auth_lite_model
{
    ...
    public function identity_available($identity = FALSE, $user_id = FALSE)
    {
        ...
        //customization for phpVMS
        $sql_where .= ' AND uacc_is_deleted = \'0\'';
    ...
    public function email_available($email = FALSE, $user_id = FALSE)
    {
        ...
        //customization for phpVMS
        return $this->db->where(array($this->auth->tbl_col_user_account['email'] => $email, 'uacc_is_deleted' => '0'))
                ->count_all_results($this->auth->tbl_user_account) == 0;
    }
    ...
    public function username_available($username = FALSE, $user_id = FALSE)
    {
        ...
        //customization for phpVMS
        return $this->db->where(array($this->auth->tbl_col_user_account['username'] => $username, 'uacc_is_deleted' => '0'))
                ->count_all_results($this->auth->tbl_user_account) == 0;
    }
    ...
    public function send_email($email_to = NULL, $email_title = NULL, $data = NULL, $template = NULL)
    {
        ...
        $this->email->clear();
        $this->email->initialize(array('mailtype' => $this->settings_model->setting_value('EMAIL_TYPE')));
        $this->email->set_newline("\r\n");
        $this->email->from($this->settings_model->setting_value('ADMINEMAIL'), $this->settings_model->setting_value('SITENAME'));
        $this->email->to($email_to);
        $this->email->subject($this->settings_model->setting_value('SITENAME') . " " . $email_title);
